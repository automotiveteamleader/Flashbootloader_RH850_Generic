...
#if ( FBLKBAPI_FRAMEDIAG_UDS1_VERSION != 0x0402u ) || \
...
void ApplDiagUserRoutine( vuint8 * pbDiagData, tTpDataType diagReqDataLen )
{
   ...
   (void)diagReqDataLen;
#endif
   /* TODO: Implement user specific RID handlers here. */
   ...
   for (i = 0u; i < didNo; i++)
   {
      ...
      if (outIdxAdd != 0u)
      {
         ...
         switch (currentDid)
         {
            ...
            {
               /* Version of FBL */
               diagData[2] = (vuint8)((FBL_VERSION >> 8) & 0xFFu);
 ...
 *   \return        kDiagReturnValidationOk: This is special return code for
 *                                 Write Fingerprint. It will be returned, if
 *                                 the fingerprint is written and no error occurs.
 **********************************************************************************************************************/
/* PRQA S 3673 1 */ /* MD_FblKbApi_3673 */
