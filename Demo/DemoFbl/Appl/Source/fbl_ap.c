...
#  if defined( FBL_CAN_0 )
   /* Channel 0 Rx */
   #if 0
   FBL_PFCAE0    &=  FblInvert16Bit(1 << 1);    /* Clear bit 1 of PFCAE0 */
   FBL_PFCE0     &=  FblInvert16Bit(1 << 1);    /* Clear bit 1 in PFCE0 for CAN mode */
   FBL_PFC0      |=   (1 << 1);                 /* Set bit 1 to select CAN mode */
   FBL_PM0       |=   (1 << 1);                 /* Set Pin 1 as input */
   FBL_PMC0      |=   (1 << 1);                 /* Enable alternative port function */

   /* Channel 0 Tx */
   FBL_PFCAE0    &=  FblInvert16Bit(1 << 0);    /* Clear bit 0 of PFCAE0 */
   FBL_PFCE0     &=  FblInvert16Bit(1 << 0);    /* Clear bit 0 in PFCE0 for CAN mode */
   FBL_PFC0      |=   (1 << 0);                 /* Set bit 0 to select CAN mode */
   FBL_PM0       &=  FblInvert16Bit(1 << 0);    /* Set Pin 0 as output */
   FBL_PMC0      |=   (1 << 0);                 /* Enable alternative port function */
   #endif
   
   
   
   /* Channel 0 Rx *//*P2_0*/
   FBL_PFCAE2    &=  FblInvert16Bit(1 << 0);    /* Clear bit 0 of PFCAE2 */
   FBL_PFCE2     &=  FblInvert16Bit(1 << 0);    /* Clear bit 0 in PFCE2 for CAN mode */
   ...
   /* Channel 0 Tx *//*P2_1*/
   FBL_PFCAE2    &=  FblInvert16Bit(1 << 1);    /* Clear bit 1 of PFCAE2 */
   FBL_PFCE2     &=  FblInvert16Bit(1 << 1);    /* Clear bit 1 in PFCE2 for CAN mode */
...
#  endif /* FBL_CAN_0 */
#  if defined( FBL_CAN_1 )
   /* Channel 1 Rx*/
   FBL_PFCAE0    &=  FblInvert16Bit(1 << 2);    /* Clear bit 2 of PFCAE0 */
   FBL_PFCE0     &=  FblInvert16Bit(1 << 2);    /* Clear bit 2 in PFCE0 for CAN mode */
   FBL_PFC0      |=   (1 << 2);                 /* Set bit 2 to select CAN mode */
   FBL_PM0       |=   (1 << 2);                 /* Set Pin 2 as input */
   FBL_PMC0      |=   (1 << 2);                 /* Enable alternative port function */

   /* Channel 1 Tx */
   FBL_PFCAE0    &=  FblInvert16Bit(1 << 3);    /* Clear bit 3 of PFCAE0 */
   FBL_PFCE0     &=  FblInvert16Bit(1 << 3);    /* Clear bit 3 in PFCE0 for CAN mode */
   FBL_PFC0      |=   (1 << 3);                 /* Set bit 3 to select CAN mode */
   FBL_PM0       &=  FblInvert16Bit(1 << 3);    /* Set Pin 3 as output */
   FBL_PMC0      |=   (1 << 3);                 /* Enable alternative port function */
#  endif /* FBL_CAN_1 */
#  if defined( FBL_CAN_2 )
   /* Channel 2 Rx */
   FBL_PFCAE0    &=  FblInvert16Bit(1 << 5);    /* Clear bit 5 of PFCAE0 */
   FBL_PFCE0     &=  FblInvert16Bit(1 << 5);    /* Clear bit 5 in PFCE0 for CAN mode */
   FBL_PFC0      &=  FblInvert16Bit(1 << 5);    /* Set bit 5 to select CAN mode */
   FBL_PM0       |=   (1 << 5);                 /* Set Pin 5 as input */
   FBL_PMC0      |=   (1 << 5);                 /* Enable alternative port function */

   /* Channel 2 Tx */
   FBL_PFCAE0    &=  FblInvert16Bit(1 << 4);    /* Clear bit 4 of PFCAE0 */
   FBL_PFCE0     &=  FblInvert16Bit(1 << 4);    /* Clear bit 4 in PFCE0 for CAN mode */
   FBL_PFC0      &=  FblInvert16Bit(1 << 4);    /* Set bit 4 to select CAN mode */
   FBL_PM0       &=  FblInvert16Bit(1 << 4);    /* Set Pin 4 as output */
   FBL_PMC0      |=   (1 << 4);                 /* Enable alternative port function */
#  endif /* FBL_CAN_2 */
#  if defined( FBL_CAN_3 )
   /* Channel 3 Rx */
   FBL_PFCAE1    &=  FblInvert16Bit(1 << 2);    /* Clear bit 2 of PFCAE0 */
   FBL_PFCE1     &=  FblInvert16Bit(1 << 2);    /* Clear bit 2 in PFCE0 for CAN mode */
   FBL_PFC1      &=  FblInvert16Bit(1 << 2);    /* Set bit 2 to select CAN mode */
   FBL_PM1       |=   (1 << 2);                 /* Set Pin 2 as input */
   FBL_PMC1      |=   (1 << 2);                 /* Enable alternative port function */

   /* Channel 3 Tx */
   FBL_PFCAE1    &=  FblInvert16Bit(1 << 3);    /* Clear bit 3 of PFCAE0 */
   FBL_PFCE1     &=  FblInvert16Bit(1 << 3);    /* Clear bit 3 in PFCE0 for CAN mode */
   FBL_PFC1      &=  FblInvert16Bit(1 << 3);    /* Set bit 3 to select CAN mode */
   FBL_PM1       &=  FblInvert16Bit(1 << 3);    /* Set Pin 3 as output */
   FBL_PMC1      |=   (1 << 3);                 /* Enable alternative port function */
#  endif /* FBL_CAN_3 */
#  if defined( FBL_CAN_4 )
   /* Channel 4 Rx */
   FBL_PFCAE1    &=  FblInvert16Bit(1 << 12);   /* Clear bit 12 of PFCAE0 */
   FBL_PFCE1     &=  FblInvert16Bit(1 << 12);   /* Clear bit 12 in PFCE0 for CAN mode */
   FBL_PFC1      &=  FblInvert16Bit(1 << 12);   /* Set bit 12 to select CAN mode */
   FBL_PM1       |=   (1 << 12);                /* Set Pin 12 as input */
   FBL_PMC1      |=   (1 << 12);                /* Enable alternative port function */

   /* Channel 4 Tx */
   FBL_PFCAE1    &=  FblInvert16Bit(1 << 13);   /* Clear bit 3 of PFCAE0 */
   FBL_PFCE1     &=  FblInvert16Bit(1 << 13);   /* Clear bit 3 in PFCE0 for CAN mode */
   FBL_PFC1      &=  FblInvert16Bit(1 << 13);   /* Set bit 3 to select CAN mode */
   FBL_PM1       &=  FblInvert16Bit(1 << 13);   /* Set Pin 3 as output */
   FBL_PMC1      |=   (1 << 13);                /* Enable alternative port function */
#  endif /* FBL_CAN_4 */
   /* Watchdog test pin *//*vcshwel*/
  // FBL_P0        &=   FblInvert16Bit(1 << 7);   /* Set Pin 0.7 to low */
  // FBL_PM0       &=   FblInvert16Bit(1 << 7);   /* Set Pin 0.7 as output */

#else
# error "ToDo: Add CAN I/O port initialization for your hardware"
#endif /* FBL_ENABLE_VECTOR_HW */
   /* Initialize EEPROM-Driver */
   
   
   //FBL_P(5)       &=  FblInvert16Bit(0 << 10); /* Set Pin 10 to low */
   //FBL_PM(5)      &=  FblInvert16Bit(1 << 10);    /* Set Pin 10 as output */
  
   //FBL_PMC(5)      &= FblInvert16Bit(1 << 10); /* Choose port mode and not an alternative mode */
   
   
   //FBL_PMC(5)      &=  FblInvert16Bit(0 << 10);              /* DISABLE  able alternative port function */  
  // FBL_PM(5)       &=  FblInvert16Bit(0 << 10);    /* Set Pin 1 as output */
   FBL_PM(5)       &= 0xfbff;
   FBL_PM(5)       &= FblInvert16Bit(0 << 10);
  // FBL_P(5)        =  0xffff;
...
void ApplFblTask( void )
{
   ...
   if (flashlight > 500)
   {
      flashlight = 0;
#if defined( FBL_ENABLE_VECTOR_HW )
     // test=FblCanTransmit(kFblTpTxHandle);/* Toggle LED */
#endif
   }
}
...
{
//#if defined( FBL_ENABLE_MULTIPLE_NODES )               //vcshwel
   vuint8 currentEcu = 1;
//#endif
