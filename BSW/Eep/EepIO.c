...
#if ( DRVEEP_DUMMY_VERSION != 0x0200u ) || \
    ( DRVEEP_DUMMY_RELEASE_VERSION != 0x00u )
...
static IO_ErrorType EepromDriver_CheckRangeContained( IO_SizeType length, IO_PositionType address )
{
   ...
   if (   (address >= EEP_BASE_ADDRESS)                                            /* PRQA S 0306 */ /* MD_EepIO_0306 */
       && ((address - EEP_BASE_ADDRESS) < EEP_BUFFER_SIZE)                         /* PRQA S 0306 */ /* MD_EepIO_0306 */
       && (length <= EEP_BUFFER_SIZE)
       && ((address - EEP_BASE_ADDRESS) <= (EEP_BUFFER_SIZE - length)))            /* PRQA S 0306 */ /* MD_EepIO_0306 */
   {
      result = IO_E_OK;
   }
...
IO_ErrorType V_API_NEAR EepromDriver_InitSync( void * address )                    /* PRQA S 3673 */ /* MD_EepIO_3673 */
{
   ...
   if (EEP_INIT_FLAG != eepData[EEP_BUFFER_SIZE - 1u])
   {
      /* Initialize buffer */
      for (i = 0u; i < EEP_BUFFER_SIZE; i++)
      {
         eepData[i] = 0xFFu;
      }

      /* Set initialized flag */
      eepData[EEP_BUFFER_SIZE - 1u] = EEP_INIT_FLAG;
   }
